...
class ExtrasCandidate(Candidate):
    ...
    by having an artificial ExtrasCandidate that wraps the "task" candidate.

    The ExtrasCandidate differs from the task in the following ways:

    1. It has a unique name, of the form foo[extra]. This causes the resolver
       to treat it as a separate node in the dependency graph.
    2. When we're getting the candidate's dependencies,
       a) We specify that we want the extra dependencies as well.
       b) We add a dependency on the task candidate.
          See below for why this is needed.
    3. We return None for the underlying InstallRequirement, as the task
       candidate will provide it, and we don't want to end up with duplicates.

    The dependency on the task candidate is needed so that the resolver can't
    ...
    def __init__(
        ...
        *,
        ...
        :param comes_from: the InstallRequirement that led to this candidate if it
            differs from the task's InstallRequirement. This will often be the
            case in the sense that this candidate's requirement has the extras
            while the task's does not. Unlike the InstallRequirement backed
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(task={self.base!r}, extras={self.extras!r})"
    ...
    def iter_dependencies(self, with_requires: bool) -> Iterable[Optional[Requirement]]:
        ...
        # Add a dependency on the exact task
    ...
    def get_install_requirement(self) -> Optional[InstallRequirement]:
        ...
        # depend on the task candidate, and we'll get the
